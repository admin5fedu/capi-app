...
interface KhachHangDetailViewProps {
  data: DoiTac;
}

const KhachHangDetailView: React.FC<KhachHangDetailViewProps> = ({ data }) => {
  const contactItems = [
    { label: 'Người liên hệ', value: data.ten_doi_tac, icon: User },
  ...
  return (
    <div className="space-y-8 p-6">
      <div className="flex items-center gap-4 pb-6 border-b border-slate-100">
        <div className="w-16 h-16 rounded-2xl bg-blue-50 text-blue-500 flex items-center justify-center shadow-lg shadow-blue-500/10">
          <User size={32} />
        </div>
        <div>
          <h3 className="text-xl font-bold text-slate-900">{data.ten_doi_tac}</h3>
          <p className="text-sm text-slate-500 mt-1">{data.cong_ty || 'Khách hàng cá nhân'}</p>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-[11px] font-medium text-slate-400 uppercase tracking-tighter">Nhóm khách hàng</p>
                <Badge variant="info" className="mt-1">
                  {data.zz_capi_nhom_doi_tac?.ten_nhom || 'Vãng lai'}
                </Badge>
              </div>
            </div>
            {data.thong_tin_khac && (
...
export default KhachHangDetailView;
