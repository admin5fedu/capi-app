...
export function exportBaoCaoTaiKhoanToPDF(data: BaoCaoTaiKhoanData, filters?: BaoCaoTaiKhoanFilters) {
  ...
  doc.text('BÁO CÁO TÀI KHOẢN', 14, 15)
  ...
  doc.setFont('helvetica', 'normal')
  const summaryY = 40
  doc.text(`Số dư đầu kỳ: ${formatCurrency(data.summary.soDuDauKy)}`, 14, summaryY)
  doc.text(`Tổng thu: ${formatCurrency(data.summary.tongThu)}`, 14, summaryY + 5)
  doc.text(`Tổng chi: ${formatCurrency(data.summary.tongChi)}`, 14, summaryY + 10)
  doc.text(`Tồn cuối: ${formatCurrency(data.summary.tonCuoi)}`, 14, summaryY + 15)
  doc.text(`Số lượng giao dịch: ${data.summary.soLuongGiaoDich.toLocaleString('vi-VN')}`, 14, summaryY + 20)
  ...
  doc.save(`Bao-cao-tai-khoan-${dateStr}.pdf`)
}
