...
interface BaoCaoTableLoaiTienMobileProps {
  data: BaoCaoGroupedByLoaiTien[]
}

export function BaoCaoTableLoaiTienMobile({ data }: BaoCaoTableLoaiTienMobileProps) {
    ...
    <div className="space-y-3 p-4">
      {data
        ...
        .map((item) => {
          ...
          return (
            <Card key={item.loaiTien}>
              <CardContent className="p-4 space-y-2">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center rounded-full bg-muted px-2 py-1 text-xs font-medium">
                    {item.loaiTien}
                  </span>
                  <div className="font-semibold text-base">{item.loaiTien}</div>
                </div>
